...
class Population:
    """Represent the population of a group of lantern fish."""

    def __init__(self, raw_pop: str):
        """Create a Population from a string representation of it's population."""
        pop = [int(val) for val in raw_pop.strip().split(",")]
        self.counter = Counter(pop)

    def evolve(self) -> None:
        """Change the population as if one day has passed."""
        new_counter = Counter()

        for timer, nb in self.counter.items():
            if timer == 0:
                new_counter.update({6: nb, 8: nb})
            else:
                new_counter.update({timer - 1: nb})

        self.counter = new_counter


def day6(raw_pop: str) -> Population:
    """Solve day 5 puzzle part 1 for any valid input."""
    return Population(raw_pop)
